...
public class RoleController {
	...
	@Operation(
			summary = "List roles",
			description = "Returns one page of roles for display in a table. Request: pageIndex (zero-based), pageSize (all rows if omitted), filters (field = RoleDTO field name, searchOperation, value in field1, upper bound in field2 for BETWEEN) and orders (fieldName, direction ASC or DESC); without orders the roles are sorted by code. Response: name (translated table title), column (columns to show, the identifier left out), allColumns (every RoleDTO field with its translated label), list (roles), total and numberOfPages. Labels are translated to the language from the X-Language header.",
			...
			responses = {
					@ApiResponse(responseCode = "200", description = "Page of roles",
	...
	@Operation(
			summary = "Get role",
			description = "Returns one role by identifier, used to fill the edit form.",
			responses = {
					@ApiResponse(responseCode = "200", description = "Role found",
							content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = RoleDTO.class))),
					@ApiResponse(responseCode = "400", description = "Role with the given identifier does not exist",
							content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = ErrorDetail.class))),
					@ApiResponse(responseCode = "401", description = "No valid session",
							content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = ErrorDetail.class))),
					@ApiResponse(responseCode = "403", description = "Current user does not have the admin role",
	...
	public Response getRole(
			@Parameter(description = "Role identifier", required = true) @PathParam("id") UUID id
			) {
	...
	@Operation(
			...
			description = "Creates a new role when id is empty, otherwise updates the role with that id. The code is required and must be unique; the description is optional. Changing the code changes what @RolesAllowed on the endpoints matches, so an endpoint can become unreachable for users who had that role.",
			requestBody = @RequestBody(
					description = "Role data; id empty for a new role",
					required = true,
					content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = RoleDTO.class))),
			responses = {
					@ApiResponse(responseCode = "200", description = "Role saved",
							content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = RoleDTO.class))),
					@ApiResponse(responseCode = "400", description = "Code is missing, another role already uses that code, or the role to update does not exist",
							content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = ErrorDetail.class))),
					@ApiResponse(responseCode = "401", description = "No valid session",
							content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = ErrorDetail.class))),
					@ApiResponse(responseCode = "403", description = "Current user does not have the admin role",
	...
	@Operation(
			summary = "Delete role",
			description = "Deletes the role with the given identifier. The role is removed from every user that had it, so those users lose the access that the role granted. The response body is empty.",
			responses = {
