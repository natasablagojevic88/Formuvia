...
public class ExportTableController {
	...
	@Operation(summary = "Export table to Excel", description = "Builds an Excel file (.xlsx) from a table that was already read through a table endpoint. The table title becomes the first row, the column descriptions the second, and the rows follow in the order they were sent; numbers, dates and boolean values are written in the cell format of their column type, with boolean values translated to the language of the request. The client decides what ends up in the file: it usually asks for the table without paging, so that the export holds every row matching the current filters and sort order, not just the visible page. The file name is returned in the Content-Disposition header, which is also exposed to the browser for cross-origin requests. Requires a valid session.", requestBody = @RequestBody(description = "Table to export, exactly as returned by the table endpoint (title, columns and rows)", required = true, content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = DatabaseTable.class))), responses = {
			@ApiResponse(responseCode = "200", description = "Excel file, with the file name in the Content-Disposition header", content = @Content(mediaType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", schema = @Schema(type = "string", format = "binary"))),
			@ApiResponse(responseCode = "400", description = "Table cannot be exported, for example when a column or value is not in the expected format", content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = ErrorDetail.class))),
	...
	public Response getExportTable(@SuppressWarnings("rawtypes") DatabaseTable databaseTable) {
